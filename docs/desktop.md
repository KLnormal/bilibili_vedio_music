...
- 本次下载：在“任务与视频”页展开本次下载筛选，可临时覆盖时长和发布时间，
  时长范围支持 0–14400 秒（最多 4 小时），并可取消“启用时长筛选”以关闭本次时长过滤；
  ...
  可重新载入当前 UP 或全局默认规则。
- 单个视频直下：输入一个 YouTube 视频 URL/ID 或 Bilibili BV 号后，可执行“直下预览”或“直接下载”。
  该路径固定显示 `READY 1` 并忽略所有筛选规则；文件保存到下载根目录的 `Direct` 文件夹（YouTube 为 `YouTube/Direct`）。
...
- YouTube 下载进度：任务页会显示当前视频、队列序号、下载速度和字节进度；网络重试
  有上限，程序异常退出后遗留的下载记录会在下一次任务中自动恢复。
- “已下载”页面在任务运行期间会实时刷新；开始 YouTube 下载前会自动核对文件是否存在，
  缺失文件会重新进入下载队列。
...
2026-08-30：修复 YouTube 来源在“任务与视频”页默认“全部 UP”时的扫描触发；空选择
现在会传递为 `None` 并扫描全部启用频道，不再尝试解析字面量 `"None"`。
同时使“UP 管理”下载按钮遵循设置页的清晰度/媒体类型默认值，新增 YouTube Cookie
配置与登录态检查入口。
