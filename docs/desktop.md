...
- 本次下载：在“任务与视频”页展开本次下载筛选，可临时覆盖时长和发布时间，
  ...
  黑名单筛选，以及通过“设置”弹窗批量增删当前 UP 的关键词；不修改 UP 默认规则。
- 筛选布局：筛选标题、时长和发布时间各自占用独立区域；操作按钮、下载状态、
  扫描状态依次排列，避免在高分辨率及缩放显示器上重叠。
...
2026-08-28：调整“任务与视频”的筛选区内边距、行距和最小高度；操作按钮及状态
区域改为明确分行；新增并列黑名单管理控件、启用/禁用持久化和弹窗编辑，以及布局
与配置回归测试。
