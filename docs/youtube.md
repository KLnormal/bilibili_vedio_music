...
桌面下载页的时长范围为 0–14400 秒（最多 4 小时）；取消“启用时长筛选”后，
本次 YouTube 预览和下载不会按时长排除视频。
任务页还支持单个 YouTube 视频直下：输入视频 ID 或 URL 后执行“直下预览”会显示 `READY 1`，
且不受频道、时长、日期、黑名单或指定名单限制；文件统一保存到 `YouTube/Direct`。
