...
8. **下载状态只针对当前下载目录。** 切换目录后递归识别有效的 `[BV号].mp4/.m4a`，
   旧目录状态不会阻止新目录补全；切回旧目录时重新按文件重建状态。
...
`check`/`scan` 会同步当前下载目录中的有效文件。目录切换可在桌面“设置”页通过
输入框或“浏览”按钮完成；保存后后台验证目录可写并立即刷新任务状态。CLI 也可用
`python main.py check [mid] --type video|audio` 手动触发当前目录同步。
